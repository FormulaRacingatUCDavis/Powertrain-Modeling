function [WaterOutletTemp1, WaterOutletTemp2, NumberOfTransferUnits, Effectiveness, AttainableHeatTransferRate, RadiatorPressDrop]....
     = RadiatorThermalCircuitReqHT(WaterInletTemp, AirInletTemp, AirFreeStreamVelocity, WaterMassFlow)
...
WaterOutletTemp =  WaterInletTemp - 2;
...
WaterMassFlow         = WaterMassFlow;  % Water volumetric flow (L/min -> m3/s)
WaterVolumetricFlow   = WaterMassFlow./WaterDensity;                   %Mass flow water (kg/s)
...
WaterPrandtlNumber  = (WaterHeatCapacity.*WaterDynamicViscosity)./WaterThermalConductivity;
  ...
  AirHeatCapacity     = py.CoolProp.CoolProp.PropsSI('C','P',101325,'T',AirMeanTemp,'Air');
  AirConductivity     = py.CoolProp.CoolProp.PropsSI('L','P',101325,'T',AirMeanTemp,'Air');
  AirDynamicViscosity = py.CoolProp.CoolProp.PropsSI('V','P',101325,'T',AirMeanTemp,'Air');
  AirDensity          = py.CoolProp.CoolProp.PropsSI('D','P',101325,'T',AirMeanTemp,'Air'); 
  AirPrandtlNumber = (AirDynamicViscosity*AirHeatCapacity)/AirConductivity;

  
  

AirMassFlow     = AirDensity*RadiatorZ*RadiatorY*AirFreeStreamVelocity; % Air mass flow from CFD (kg/s)
...
%Thermal Circuit From Water to Tuber Surface For one tube
    ...
    SurfaceHeatCapacity  = py.CoolProp.CoolProp.PropsSI('C','P',101325,'T',SurfaceTemp,'Air');
    SurfaceConductivity  = py.CoolProp.CoolProp.PropsSI('L','P',101325,'T',SurfaceTemp,'Air');
    SurfaceViscosity     = py.CoolProp.CoolProp.PropsSI('V','P',101325,'T',SurfaceTemp,'Air');
...
RadiatorPressDrop = 4.*WaterFrictionFactor.*(RadiatorZ./TubeInnerDiamter).*...
