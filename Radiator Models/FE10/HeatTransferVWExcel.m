%% Could not find equation to calculate for humid air dynamic viscosity and thermal conducticity
% Predict Result error of less than 10%
...
function[WaterHeatCapacityRate,WaterThermalConductivity,WaterReynoldsNumber,WaterPrandtlNumber,WaterMassFlow,WaterHeatCapacity] = WaterProperty(WaterMeanTemp)
    WaterData = readmatrix('Physical Characteristics of Water.xlsx');
    xWater = WaterData(:,1);
    y.CpWater = WaterData(:,7);
    y.RhoWater = WaterData(:,4);
    y.DynamicViscosityWater = WaterData(:,10);
    y.ThermalConductivityWater = WaterData(:,12);
    
    WaterDensity = interp1(xWater, y.RhoWater, WaterMeanTemp);
    WaterHeatCapacity = interp1(xWater, y.CpWater, WaterMeanTemp)*1000;
    WaterThermalConductivity = interp1(xWater,y.ThermalConductivityWater,WaterMeanTemp);
    WaterDynamicViscosity = interp1(xWater,y.DynamicViscosityWater,WaterMeanTemp);
...
function[AirHeatCapacity,AirThermalConductivity,AirReynoldsNumber,AirPrandtlNumber,AirHeatCapacityRate,AirDensity,AirFreeStreamVelocity] = HumidAirProperty(AirMeanTemp,RelativeHumidity,AirMassFlow)
    ...
    WaterData = readmatrix('Physical Characteristics of Water.xlsx');
    xWater = WaterData(:,1);
    y.SaturatedPressure = WaterData(:,3);

    DryAirData = readmatrix('Physical Characteristics of Air.xlsx');    
    xAir                     = DryAirData(:,1);
    y.CpAir                  = DryAirData(:,3);
    y.RhoAir                 = DryAirData(:,2);
    y.DynamicViscosityAir    = DryAirData(:,5);
    y.ThermalConductivityAir = DryAirData(:,4);

    AtmosphericPressure = 101325; %Pa
    SaturatedPressure = interp1(xWater, y.SaturatedPressure, AirMeanTemp);
    VaporPressure = RelativeHumidity/100*SaturatedPressure; 
    HumidityRatio = 0.622*VaporPressure/(AtmosphericPressure-VaporPressure);

    DryAirDensity   = interp1(xAir, y.RhoAir, AirMeanTemp);
    AirDensity      = DryAirDensity*(1+HumidityRatio)/(1+1.609*HumidityRatio);

    AirDynamicViscosity    = interp1(xAir, y.DynamicViscosityAir, AirMeanTemp);
    AirThermalConductivity = interp1(xAir, y.ThermalConductivityAir, AirMeanTemp);
    
    DryAirHeatCapacity     = interp1(xAir, y.CpAir, AirMeanTemp)*1000;
    HeatCapacityWaterVapor = 1820; % J/KgK
    AirHeatCapacity        = (DryAirHeatCapacity)+ HumidityRatio*(HeatCapacityWaterVapor); 
...
function[SurfacePrandtl] = SurfaceAirProperty(SurfaceTemp,RelativeHumidity)
    ...
    AtmosphericPressure = 101325; %Pa
    SaturatedPressure = interp1(xWater, y.SaturatedPressure, SurfaceTemp);
    VaporPressure = RelativeHumidity/100*SaturatedPressure; 
    HumidityRatio = 0.622*VaporPressure/(AtmosphericPressure-VaporPressure);

    SurfaceDynamicViscosity    = interp1(xAir, y.DynamicViscosityAir, SurfaceTemp);
    SurfaceThermalConductivity = interp1(xAir, y.ThermalConductivityAir, SurfaceTemp);
    
    SurfaceDryAirHeatCapacity     = interp1(xAir, y.CpAir, SurfaceTemp)*1000;
    HeatCapacityWaterVapor = 1820; % J/KgK
    SurfaceHeatCapacity        = (SurfaceDryAirHeatCapacity)+ HumidityRatio*(HeatCapacityWaterVapor); 
