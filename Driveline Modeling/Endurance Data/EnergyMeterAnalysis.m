...
% Blake Christierson - 16 May 2020
...
%Revision: Elliot Locke - 29 May 2020
%   Cropped start and stop of Endurance stints and added relevant RMS
%   power, voltage and caculations with appropriate scaling. 
...
%% Data Import
Stint(1).Raw = StintImport('Cropped_FE6_Endurance_stint_1.csv');
Stint(2).Raw = StintImport('Cropped_FE6_Endurance_stint_2.csv');


% Raw Power Calculation [kW]
Stint(1).Raw.Power = Stint(1).Raw.Voltage.*Stint(1).Raw.Current./1000 %kW
Stint(2).Raw.Power = Stint(2).Raw.Voltage.*Stint(2).Raw.Current./1000 %kW

Bruh = cumsum(Stint(1).Raw.Power)
for i = 1:length(Bruh)
Bruh(i) = Bruh(i) / i;
i = i+1;
end

Bro = cumsum(Stint(2).Raw.Power)
for i = 1:length(Bro)
Bro(i) = Bro(i) / i;
i = i+1;
end
...
plot(Bruh); 
plot(Bro); hold off;
legend('Stint1','Stint2');
xlabel('Sample (10hz)');
ylabel('Power (kW)');
title('Time Average Power Output, 2019 Endurance')
grid on;


PowerRMS = (rms(Stint(1).Raw.Power) + rms(Stint(2).Raw.Power))/2; %kW
PowerMean = (mean(Stint(1).Raw.Power) + mean(Stint(2).Raw.Power))/2; %kW
ScaledPower = (100/60)*PowerRMS; %kW, 60% to 100% Torque scaling
ScaledPowerFOS = (1.2)*ScaledPower; %kW, 1.2 Factor of Safety

VoltageRMS = (rms(Stint(1).Raw.Voltage) + rms(Stint(2).Raw.Voltage))/2; %V

CurrentMean = (mean(Stint(1).Raw.Current) + mean(Stint(2).Raw.Current))/2;
CurrentRMS = (rms(Stint(1).Raw.Current) + rms(Stint(2).Raw.Current))/2; %A
ScaledCurrentRMS = (100/60)*CurrentRMS; %A, 60% to 100% Torque scaling
ScaledCurrentMean = (100/60)*CurrentMean;
